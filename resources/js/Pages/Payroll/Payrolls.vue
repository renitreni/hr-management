...
const filter = (column = "id", direction = "desc") => {
...
function sortThis($column, $direction) {
      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <Card class="!mt-0">
            ...
            <div class="w-1/2" dir="ltr">
              ...
              <ul class="ul-checkbox mb-1">
                <li class="li-checkbox">
                  <div class="ul-li-div-radio">
                    <input
                      ...
                      v-model="status"
                      name="list-radio"
                      class="li-radio-input"
                    />
                    <label
                      for="horizontal-list-radio-all"
                      class="li-radio-label"
                      >{{ __("All") }}</label
                    >
                  </div>
                </li>
                <li class="li-checkbox">
                  <div class="ul-li-div-radio">
                    <input
                      id="horizontal-list-radio-pending"
                      type="radio"
                      value="pending"
                      v-model="status"
                      name="list-radio"
                    ...
                    />
                    <label
                      for="horizontal-list-radio-pending"
                      class="li-radio-label"
                      >{{ __("Pending") }}</label
                    >
                  </div>
                ...
                <li class="li-checkbox">
                  <div class="ul-li-div-radio">
                    <input
                      id="horizontal-list-radio-reviewed"
                      type="radio"
                      value="reviewed"
                      v-model="status"
                      name="list-radio"
                    ...
                    />
                    <label
                      for="horizontal-list-radio-reviewed"
                      class="li-radio-label"
                      >{{ __("Reviewed") }}</label
                    >
                  </div>
                ...
                <li class="li-checkbox">
                  <div class="ul-li-div-radio">
                    ...
                    />
                    <label
                      for="horizontal-list-radio-paid"
                      class="li-radio-label"
                      >{{ __("Paid") }}</label
                    >
                  </div>
          ...
          <Table
            ...
            :showingNumber="payrolls.data.length"
            :totalNumber="payrolls.total"
          >
            <template #Head>
              <TableHead @click="sortThis('id', direction)">{{
                __("Payroll ID")
              }}</TableHead>
              <TableHead @click="sortThis('employee_name', direction)">{{
                __("Employee Name")
              }}</TableHead>
              <TableHead @click="sortThis('total_payable', direction)">{{
                __("Total Amount")
              }}</TableHead>
              <TableHead @click="sortThis('due_date', direction)">{{
                __("Due Date")
              }}</TableHead>
              <TableHead>{{ __("Status") }}</TableHead>
              <TableHead v-if="$page.props.auth.user.roles.includes('admin')">{{
                __("Action")
              }}</TableHead>
            </template>
            ...
            <template #Body>
              <TableRow v-for="payroll in payrolls.data" :key="payroll.id">
                <TableBodyHeader
                  :href="route('payrolls.show', { id: payroll.id })"
                  >{{ payroll.id }}</TableBodyHeader
                >
                <TableBodyHeader
                  :href="route('payrolls.show', { id: payroll.id })"
                  >{{ payroll.employee_name }}</TableBodyHeader
                >
                <TableBody :href="route('payrolls.show', { id: payroll.id })"
                  >{{ payroll.currency }} {{ payroll.total_payable }}</TableBody
                >
                <TableBody :href="route('payrolls.show', { id: payroll.id })">{{
                  payroll.due_date
                }}</TableBody>
                ...
                <TableBodyAction
                  ...
                  :href="route('payrolls.edit', { id: payroll.id })"
